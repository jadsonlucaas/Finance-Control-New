function ensureEntradasCycleFilter() {
            const actions = document.querySelector('#view-entradas .mobile-section-actions');
            if (!actions || document.getElementById('entradas-cycle-filter')) return;
            const select = document.createElement('select');
            select.id = 'entradas-cycle-filter';
            select.className = 'text-xs rounded-lg border border-surfaceLight bg-surfaceLight/40 text-textSecondary px-3 py-1.5';
            select.innerHTML = `
                <option value="INICIO_MES">Início do mês</option>
                <option value="QUINZENA">Quinzena</option>
            `;
            select.onchange = () => renderEntradas();
            actions.insertBefore(select, document.getElementById('entradas-search'));
        }

        function getEntradasCycleFilterValue() {
            ensureEntradasCycleFilter();
            return document.getElementById('entradas-cycle-filter')?.value || 'INICIO_MES';
        }

        function mapEntradaToCycleView(entry, cycleFilter) {
        ...
        renderEntradas = function () {
            ...
            list.innerHTML = entries.map((entry) => {
                ...
                return `<div class="glass rounded-2xl p-4 border border-surfaceLight">
                    ...
                    ${isQuinzenaView ? '' : `<div class="rounded-xl border border-surfaceLight bg-surfaceLight/20 p-3 mt-4">
